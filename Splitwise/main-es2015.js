  ...
  \******************************************************/
      ...
      /***/ function(module, __webpack_exports__, __webpack_require__) {
        ...
        let AddExpenseComponent = class AddExpenseComponent {
          constructor(data, dialog, dialogRef, expenseService) {
            this.data = data;
            this.dialog = dialog;
            ...
            this.filteredFriends = this.friendCtrl.valueChanges.pipe(
              ...
              Object(
                rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["map"]
              )(friend =>
                friend ? this._filter(friend) : this.allfriendsArray.slice()
              )
            );
          }
          _filter(value) {
            const filterValue = value.toLowerCase();
            return this.allfriendsArray.filter(
          ...
          onAdd() {
            ...
            if (this.splitstatus === "equally") {
              this.emails.forEach(email => {
                if (email.name === "you") {
                  this.expense.split.push({
                    owes: this.expense.amount.valueOf() / this.emails.length,
                    userEmail: this.data.userInfo.email,
                    userName: this.data.userInfo.email
                  });
                } else {
                  this.expense.split.push({
                    owes: this.expense.amount.valueOf() / this.emails.length,
              ...
              this.emails.forEach(email => {
                ...
                this.TotalAmount =
                  Number.parseInt(this.TotalAmount) +
        ...
        let ProfileComponent = class ProfileComponent {
          ...
          ngOnInit() {
            this.auth.profile().subscribe(
              user => {
                const pipe = new _angular_common__WEBPACK_IMPORTED_MODULE_3__[
                  "DatePipe"
